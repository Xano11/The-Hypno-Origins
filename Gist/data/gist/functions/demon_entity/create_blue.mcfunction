...
execute align xz run summon marker ~0.5 ~3.5 ~0.5 {Tags:["def.gist.demon","def.gist.blue","def.gist.target"]}
...
fill ~ ~-1 ~ ~ ~-1 ~ respawn_anchor[charges=0] replace respawn_anchor[charges=4]
playsound minecraft:block.respawn_anchor.deplete block @s ~ ~ ~ 1 1